...
App_Init_Second (void)
{
    ...
    AposSetInfo ("CarMaker", AppStartInfo.App_Version, "Application is starting ...");
 ...
 DoReturn:
	...
	if (SimCore.NextState == SCState_Start        /* running in main loop    */
	 || SimCore.NextState == SCState_StartWait) { /* running in start thread */
	    /* Use SysGetTime(), not SimCore.TimeWC, because the latter might
	       be way out-of-date in case of SimCore.OnlyOneSimulation! */
#if defined(CM_HIL)
	    double now = SimCore.TimeWC;
#else
	    double now = SysGetTime() - SimCore.Cycle.TimeWCRef;
#endif
	    SimCore.Anim.Wait_UntilWC = now + SimCore.Anim.Wait_Timeout;
