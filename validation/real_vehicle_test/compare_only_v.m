...
for i = 1:data_len
    ...
    if mode == 2
        if i == 1
            ...
            V = qd_i(1) * cos(q_i(3)) + qd_i(2) * sin(q_i(3));
            % lagrange nonlinear double 
            qdd_lagrange = lagrange_vehicle_fix_model(q_lagrange_multi(i-1, :)', qd_lagrange_multi(i-1, :)', delta_i, V, sat);
            qd_lagrange_multi(i, :) = qdd_lagrange * dt + qd_lagrange_multi(i-1, :)';
            q_lagrange_multi(i, :) = (qd_lagrange_multi(i, :))' * dt + q_lagrange_multi(i-1, :)';

            % lagrange nonliear single
            qdd_lagrange = lagrange_vehicle_single_fix_model(q_lagrange_single_multi(i-1, :)', qd_lagrange_single_multi(i-1, :)', delta_i, V, sat);
            qd_lagrange_single_multi(i, :) = qdd_lagrange * dt + qd_lagrange_single_multi(i-1, :)';
            q_lagrange_single_multi(i, :) = (qd_lagrange_single_multi(i, :))' * dt + q_lagrange_single_multi(i-1, :)';

            % lagrange nonliear single little steer
            qdd_lagrange = lagrange_vehicle_littlesteer_single_fix_model(q_lagrange_single_littles_multi(i-1, :)', qd_lagrange_single_littles_multi(i-1, :)', delta_i, V, sat);
            qd_lagrange_single_littles_multi(i, :) = qdd_lagrange * dt + qd_lagrange_single_littles_multi(i-1, :)';
            q_lagrange_single_littles_multi(i, :) = (qd_lagrange_single_littles_multi(i, :))' * dt + q_lagrange_single_littles_multi(i-1, :)';

            % newton linear 
            qdd_newton = newton_vehicle_model(unit_n, axle_m, ..., 
                    getNumUnit, m, Iz, Ca, lu, lg, la, q_newton_multi(i-1, :)', qd_newton_multi(i-1, :)', delta_i, V, sat);
            qd_newton_multi(i, :) = qdd_newton * dt + qd_newton_multi(i-1, :)';
            q_newton_multi(i, :) = (qd_newton_multi(i, :))' * dt + q_newton_multi(i-1, :)';

            % kinetic
            vxg = qd_i(1) + 5.862 * sin(q_i(3)) * qd_i(3);
            vyg = qd_i(2) - 5.862 * cos(q_i(3)) * qd_i(3);
            V = vxg * cos(q_i(3)) + vyg * sin(q_i(3));
            qd_kinetic_multi(i, :) = kinetic_vehicle_model(q_kinetic_multi(i-1, :)', V, delta_i);
