...
% 1 quarter_turn 2 circle 3 double lane change
% only 3 mode L > N
data_mode = 2;
...
for i = 1:data_len
    ...
    if mode == 2
        if i == 1
            ...
            % lagrange nonlinear double 
            qdd_lagrange = lagrange_fix_model(q_lagrange_multi(i-1, :)', [qd_i(1:2); qd_lagrange_multi(i-1, 3:5)'], delta_i, sat);
            qd_lagrange_multi(i, :) = qdd_lagrange * dt + [qd_i(1:2); qd_lagrange_multi(i-1, 3:5)'];
            q_lagrange_multi(i, :) = (qd_lagrange_multi(i, :))' * dt + q_lagrange_multi(i-1, :)';

            % lagrange nonliear single
            qdd_lagrange_single = lagrange_single_fix_model(q_lagrange_multi(i-1, :)', [qd_i(1:2); qd_lagrange_multi(i-1, 3:5)'], delta_i, sat);
            qd_lagrange_single_multi(i, :) = qdd_lagrange_single * dt + [qd_i(1:2); qd_lagrange_single_multi(i-1, 3:5)'];
            q_lagrange_single_multi(i, :) = (qd_lagrange_single_multi(i, :))' * dt + q_lagrange_single_multi(i-1, :)';

            % lagrange nonliear single little steer
            qdd_lagrange_single_littles = lagrange_single_fix_model(q_lagrange_multi(i-1, :)', [qd_i(1:2); qd_lagrange_multi(i-1, 3:5)'], delta_i, sat);
            qd_lagrange_single_littles_multi(i, :) = qdd_lagrange_single_littles * dt + [qd_i(1:2); qd_lagrange_single_littles_multi(i-1, 3:5)'];
            q_lagrange_single_littles_multi(i, :) = (qd_lagrange_single_littles_multi(i, :))' * dt + q_lagrange_single_littles_multi(i-1, :)';

            % newton linear 
            qdd_newton = newton_model(unit_n, axle_m, ..., 
            getNumUnit, m, Iz, Ca, lu, lg, la, q_newton_multi(i-1, :)', [qd_i(1:2); qd_newton_multi(i-1, 3:5)'], delta_i, sat);
            qd_newton_multi(i, :) = qdd_newton * dt + [qd_i(1:2); qd_newton_multi(i-1, 3:5)'];
